...
import { getAssistantSel, getValuesFromDomain, insAssistant } from "common/helpers";
...
const Assistant: FC = () => {
    ...
    useEffect(() => {
        if (applications) {
            setPagePermissions({
                view: applications["/assistant"][0],
                modify: applications["/assistant"][1],
                insert: applications["/assistant"][2],
                delete: applications["/assistant"][3],
                download: applications["/assistant"][4],
            });
        }
    }, [applications]);
    ...
    const fetchData = () => dispatch(getCollection(getAssistantSel(0)));

    // MainDataFill
    useEffect(() => {
        if (!mainResult.loading && !mainResult.error && mainResult.key === "UFN_ASSISTANTS_SEL") {
